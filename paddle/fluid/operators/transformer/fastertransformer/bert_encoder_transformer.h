...
#include "paddle/fluid/operators/transformer/fastertransformer/allocator.h"
#include "paddle/fluid/operators/transformer/fastertransformer/cuda/cuda_kernels.h"
#include "paddle/fluid/operators/transformer/fastertransformer/cuda/open_attention.h"
#include "paddle/fluid/operators/transformer/fastertransformer/encoder_transformer.h"
...
class BertEncoderTransformer:IEncoderTransformer<Traits_::OpType>
{
  ...
  int head_num_;
  int size_per_head_;
  HiddenActType hidden_act_;
  public:
  BertEncoderTransformer(const IAllocator& allocator, int batch_size, int from_seq_len, 
      int to_seq_len, int head_num, int size_per_head, const std::string& hidden_act): 
...
#ifndef NDEBUG
    PRINT_FUNC_NAME_();
    PRINTF_LOG("hidden_act is %s", hidden_act.c_str());
#endif
    if (hidden_act == "gelu") {
      hidden_act_ = HiddenActType::GELU;
    } else if (hidden_act == "relu" ) {
      hidden_act_ = HiddenActType::RELU;
    } else {
      throw std::runtime_error(std::string("Unsupported hidden_act: ") + hidden_act);
    }
  ...
  void forward() override
  {
    ...
    try{
      ...
      add_bias_act_kernelLauncher<DataType_>(inter_matmul_buf_, param_.inter_bias, m, n, param_.stream, hidden_act_);
