 ...
 */
#include "paddle/fluid/operators/transformer/fastertransformer/cuda/cuda_kernels.h"
#include <assert.h>
#include <cstdio>
#include <cstdlib>
...
template <typename T>
__global__ 
void add_bias_act(T* out, const T* bias, int m, int n, HiddenActType hidden_act)
{
  T val, reg_bias;
  ...
  for(int i = 0; i < ite; ++i)
  {
    ...
    while(row_id < m){
      ...
      if (hidden_act == HiddenActType::GELU) {
        out[tid + i * blockDim.x + row_id * n] = gelu<T>(val);
#ifndef NDEBUG
        PRINTF("hidden_act is gelu");
#endif
      } else {
#ifndef NDEBUG
        PRINTF("hidden_act is relu");
#endif
        out[tid + i * blockDim.x + row_id * n] = relu<T>(val);
      }
      row_id += gridDim.x;
    }
  }
...
template <>
__global__ 
void add_bias_act(__half* out, const __half* bias, int m, int n, HiddenActType hidden_act)
{
  half2 val, reg_bias;
  int row_id = blockIdx.x;
...
template <typename T>
void add_bias_act_kernelLauncher(T* out, const T* bias, int m, int n, cudaStream_t stream, HiddenActType hidden_act)
{
  dim3 grid(m / 4);
  dim3 block(n / 4);
  assert(block.x <= 1024);
  add_bias_act<T><<<grid, block, 0, stream>>>(out, bias, m, n, hidden_act);
}
...
template void add_bias_act_kernelLauncher<float>(
  float* out, const float* bias, int m, int n, cudaStream_t stream, HiddenActType hidden_act);
...
template void add_bias_act_kernelLauncher<__half>(
  __half* out, const __half* bias, int m, int n, cudaStream_t stream, HiddenActType hidden_act);
